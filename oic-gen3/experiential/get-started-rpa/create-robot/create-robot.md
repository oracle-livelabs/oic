# Create Robot

## Introduction

This lab walks you through the steps to create an end-to-end robot. When creating a robot, you define its trigger and the actions that it performs.

Estimated Lab Time: 45 minutes

### Objectives

In this lab, you will:

* Create a robot.
* Specify input and output triggers.
* Install the Robot Designer extension.
* Add robot actions.

### Prerequisites

This lab assumes you have:

* Completed the Robot Connection lab.
* Google Chrome with permission to manage extensions.

## Task 1: Create a Robot

When you create a robot, you define its trigger and the actions that the robot performs.

1. In the navigation pane, select **Projects**, then click the project created for this lab.

2. Click the ![Select Robot category](images/projects_select-robot-category.png "") (**Robot**) icon on the left side of the screen.

3. In the **Robot** category, go to the **Robots** box and click **Add** or **+**.

    ![Add Robot](images/projects_add-robot.png "")

    > **Note**: The **Add** button appears if you have no robots. The **+** button appears if you have at least one robot.

4. In the **Create robot** panel, fill in the following fields:
    | **Field** | **Required** | **Value** |
    | --- | ----------- | ----------- |
    | Name          | Yes | Enter a name for the robot. For example: `LiveLabs Robot` |
    | Identifier    | Yes | This entry is autogenerated based on the Name value.  |
    | Version       | Yes | Keep as default: `1.00.00` |
    | Description   | No  | Provide additional information about the robot.|
    | Keywords      | No  | Enter text that people might use to search for the robot. |

    ![Create robot panel](images/create-robot-add.png " ")

5. Click **Create**.

The Robot Designer appears with a default robot that includes an **Open Application** action.

![Robot Designer Canvas](images/robot-designer_after-create.png " ")

## Task 2: Specify Input and Output Triggers

The trigger defines the interface for the robot, including the input (incoming request) and the output (outgoing response). For this lab, we will provide a purchase order number as input, and capture the supplier name as output.

1. Select ![Create robot panel](images/click-to-edit-trigger-button-small.png) **(Click to edit trigger)**.

    The trigger panel appears.

2. On the **Input** tab, click **+ (Add)** and enter the following Input property values:

    * Name: `PONumber`
    * Type: `String` (default)

3. Select the **Output** tab, click **+ (Add)** and enter the following Output property values:

    * Name: `SupplierName`
    * Type: `String` (default)

4. Click **OK**.

## Task 3: Define Open Application Action

This action tells the robot to open a web browser and navigate to the Oracle ERP Cloud sign-in page. We will use the robot connection created in the previous lab to pull the service URL and credentials.

1. Double-click on the **Open Application** action.

    ![Select robot connections button](images/robot-designer_open-browser_edit.png " ")

2. In the **Open Browser** panel, keep the default values for **Name** and **Description**.

3. Assign the target URL dynamically using a robot connection. On the **Input** tab, select the **URL** field and click the ![Select robot connections button](images/robot-connections-button.png) (**Robot connections**) icon.

    The **Robot connections** side panel will appear, with the previously created robot connection.

4. In the **Robot connections** side panel, expand the newly created robot connection. Then drag the **URL** parameter to the matching field under the **Input** tab.

    ![Drag the URL parameter to the Input tab](images/robot-designer_open-browser_assign-url.png " ")

5. Under **Browser**, select the browser to be used by the robot. Keep all other values at their defaults.

6. Click **OK** when done, then **Save**.

7. In another browser window, open the sign-in page for the Oracle Fusion environment that the robot will work in. For example, to access Oracle Cloud ERP, your login URL pattern will be:
`https://<Fusion-domain>.fa.<data-center>.oraclecloud.com/fscmUI/faces/FuseWelcome`

8. On the canvas, select the **Open Application** action, after which you want to record.
9. On the toolbar, select **Record after the selected action**. The Smart Record panel appears. If you have not installed the recorder, **Browser extension not detected** appears; follow Task 4. Otherwise, skip Task 4.
    ![Record action](images/recordaction.png " ")

## Task 4: Install the Recorder (Optional)

The recorder is an extension that you use to build robots. Everyone who builds robots must install the recorder.

1. Select **Install browser extension**.
    ![browserextensionnotdetected action](images/browserextensionnotdetected.png " ")
2. Select **Add to Chrome**. In the pop-up, select **Add extension**. The Oracle Robot Flow Designer extension is installed in your browser. A pop-up appears when the installation is complete.
3. Refresh the browser window in which Oracle Integration is open. If you work in an Incognito window, allow the extension in Incognito windows.
    Perform these steps in an Incognito browser or a regular Chrome browser.
        a. In Google Chrome, open the **Extensions** menu and select **Manage Extensions**.
        b. In the box for the Oracle Robot Designer Extension, select Details.
        c. Enable the Allow in Incognito setting.
4. Close the **Extensions** tab and return to the robot. Click **Save**, then reopen the project.

## Task 5: Add ERP Login Actions using the Record option

While building the rest of the robot, you will record the actions that it completes using the recorder. You will navigate the Fusion pages using UI actions, such as mouse clicks and keyboard entries, which automatically generate the relevant robot actions in the flow.

1. Click the **Record after the selected option** button to start recording after the selected **Open Application** action.  ![Record button enabled](images/recordaction.png " ")

2. On the **Smart Record** panel, click **Select browser tab to target** drop-down.

3. Select the window title that corresponds to the Oracle ERP Cloud tab.

    ![Select browser tab](images/select-browser.png " ")

    This starts the recorder on the ERP Cloud login page.

4. On the **RPA Smart Recording** panel, click on **Begin Recording**.

    You will now select elements to generate associated actions in the robot.

    > **Note**: When recording actions, the recorder can start from any point in time. However, for this task we will record after the Login operation.

5. Click the **User Name** text field when the icon changes to a target. This opens a child window.
    ![erp username](images/erploginusername.png " ")
6. Click **RPA Connections**, expand **RPA ERP Connection** or the connection you created, and drag *username* from the left side to the right side under the *Value* element. Enter `bala.gupta`, or another username with permission to complete this use case, as the *Test value*, then click **Save**.
    ![enter username](images/select-username.png " ")
7. Go back to the OIC Designer and you should see the *username* element added to the RPA flow.
8. Click the **Password** text field when the icon changes to a target. This opens a child window.
    ![erp password](images/erploginpassword.png " ")
9. Click **RPA Connections**, expand **RPA ERP Connection** or the connection you created, and drag *password* from the left side to the right side under the *Value* element, then click **Save**.
10. Go back to the OIC Designer and you should see the *password* element added to the RPA flow.
11. Click the **Sign In** field when the icon changes to a target. This opens a child window; click **Save**.
    ![erp signin](images/erpsignin.png " ")

## Task 6: Add Robot ERP Actions Using the Record Option

1. From the top menu in ERP Cloud, click **Procurement** when the icon changes to a target.

    ![Click Procurement target elements](images/robot-designer_smart-record_click-procurement.png " ")

2. In the **Action Details**, keep all values at their defaults and click **Save**. This adds the `Click "Procurement"` action and opens the **Procurement** tab. If your element value is not `A`, you clicked the wrong element. Retry the action by selecting **Discard**, then click the **Procurement** target element again while using the recorder.

    ![Procurement action details](images/robot-designer_smart-record_procurement-action-details.png " ")

    > **Note:** When recording or targeting a field, hover your cursor over a UI element and note the shading that appears. While the shading is purple and the icon above the field is a magnifying glass, the recorder is still collecting information about the element. Clicking the element before it turns green will result in a regular UI interaction.

3. Click **Purchase Orders** when the entire tile is shaded and changes to a target.

    ![Select Purchase Orders tile](images/robot-designer_smart-record_click-purchase-orders.png "=50%x*")

4. In the **Action Details**, keep all values at their defaults and click **Save**. This adds the `Click "Purchase Orders"` action and opens the **Overview - Purchase Orders** screen. If your element value is not `DIV`, you clicked the wrong element. Retry the action by selecting **Discard**, then click the **Purchase Orders** target element again while using the recorder.

    ![Purchase Orders action details](images/robot-designer_smart-record_purchase-orders-action-details.png " ")

5. On the **Overview** page, move to the right of the screen and select the **Tasks** icon when the icon changes to a target.

    ![Select Tasks icon](images/erp-cloud_select-tasks.png " ")

6. In the **Action Details**, if your element value is `IMG`, click **Save** to add the `Click "Tasks"` action. If you see a different element value, you clicked the wrong UI element. Retry the action by selecting **Discard**, then click the **Tasks** icon again while using the recorder.

    ![Tasks action details](images/robot-designer_smart-record_tasks-action-details.png " ")

7. In the Tasks side panel, hover over the **Orders** section and click **Manage Orders** when the icon changes to a target.

    ![Manage Orders action details](images/robot-designer_smart-record_click-manage-orders.png " ")

8. In the **Action Details**, keep all values at their defaults and click **Save**. This adds the `Click "Manage Orders"` action.

     > **Note**: When you are ready to test the robot, a valid purchase order number is required as input. In the next steps, pause the recording and search for a valid purchase order before recording the subsequent actions.

9. Click the **Pause** button on the Smart Recording panel.

    ![Pause Smart Recording](images/robot-designer_smart-record_pause.png "" )

10. Click the **Tasks** icon again, then select **Manage Orders**. This will display the Manage Orders overview screen.

11. Click **Search** and wait until you see a list of existing purchase orders.

12. Choose one of the purchase orders with a listed `Supplier Name`, and make a note of the order number.

13. Click **Done** in the upper-right corner to close the **Manage Orders** screen.

14. Click the **Tasks** icon again, then select **Manage Orders** to display the Manage Orders overview screen.

15. Click the **Resume** button on the Smart Recording panel to resume recording UI actions.

    ![Resume Smart Recording](images/robot-designer_smart-record_resume.png " " )

16. Move to the **Order** field and click the textbox element when the icon changes to a target.

    ![Click Order textbox](images/robot-designer_smart-record_click-order-field.png " ")

17. Change the following values in the **Action Details**.

    1. Name: `Enter "PO number" Text`
    2. Target name: Add something descriptive without spaces (such as `Enter_po_number`)
    3. Value: In the field, click ![Select Flow button](images/action-details_po-number_flow-button.png) (**Flow Input/Output**) and select the **Input** property **PONumber**.
    4. Double-click the **PONumber** property to have it mapped to the **Value** field.
    5. Test value: Enter the previously captured `PO Number` in the **Test Value** textbox, which will be used in the search query to pull an actual purchase order. Example: `US164712`

    ![PO Number action details](images/action-details_po-number.png " ")

18. Click **Save**.

19. Click **Search** when the entire tile is shaded and changes to a target.

    ![Click Search button](images/robot-designer_smart-record_click-search.png " ")

20. In the **Action Details**, keep all values at their defaults and click **Save**. This adds the `Click "Search"` action and displays the search results containing the specified purchase order.

21. For the listed purchase order, move to the **Supplier** column and select the supplier name when the icon changes to a target.

    ![Click Supplier cell](images/robot-designer_smart-record_click-supplier.png " ")

22. In the **Action Details**, if your element value is not `TD`, you clicked the wrong UI element. Retry the action by selecting **Discard**, then click the **Supplier** value again while using the recorder. When the element value is `TD`, make the following changes:

    1. Action: `Get Text`
    2. Name: `Click "Get supplier name"`
    3. Target name: Add something descriptive without spaces (such as `Get_supplier_name`)
    4. Save To: In the field, click ![Select Flow button](images/action-details_output_flow-button.png) (**Output**)
    5. Double-click the **SupplierName** property to have it mapped to the **Save to** field.

    ![Assign supplier name to output](images/action-details_output_assign-supplier.png " ")

23. Click **Save**.

24. In the **Smart Recording** panel, click ![Smart Recording stop button](images/smart-recording_stop-button.png) **(Stop recording)** to end the recording.

25. In the **Robot Designer**, click **Save**.

    We will add a Log action to capture the output value of the robot.

26. In the ![Select Robot actions](images/robot-designer_select-robot-actions.png) (**Robot actions**) panel, drag the **Log** action to the end of the flow.

    ![Add Log action](images/robot-designer_add-log-action.png " ")

27. In the **Log** panel, hover over the **Message** field, click **...** (**More options**), then select **Output**. This assigns the output value to the log message.
    ![Assign Log Output Message](images/robot-designer_log-action_assign-output.png "")

28. Drag the `SupplierName` property to the **Message** field.
    ![Drag Supplier property](images/robot-designer_log-action_assign-supplier.png "")

29. Click **OK**, then **Save**.

30. Close the Robot Designer by clicking the **<** (**Go back**) button in the upper-left corner of the screen.

    The status of your robot should be **Draft**.

    ![Validate Draft state](images/projects_robot-draft.png)

You have successfully completed this lab.

## Learn More

* [11 Tips for New Robot Builders](https://docs.oracle.com/en/cloud/paas/application-integration/robotic-automation/tips-new-robot-builders.html#GUID-F6ABB846-03AA-4E62-8348-15ABFC5B777A)

## Acknowledgements

* **Author** - Ravi Chablani, Principal Product Manager - Oracle Integration
* **Last Updated By/Date** - Subhani Italapuram, Aug 2026
